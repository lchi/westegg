...
(function() {
  ...
  exports.Cache = Cache = (function() {
    ...
    Cache.prototype.unload = function(filename) {
      ...
      return this._clearFsErrorCache(realpath);
    };

    Cache.prototype.unloadAll = function() {
      var c, _i, _len, _ref, _results;
      _ref = [this.fileCache, this.fsErrorCache];
      _results = [];
    ...
    Cache.prototype.load = function(filename, options, cb) {
      ...
      return this._load(realpath, options, function(v) {
        var err, res, _ref, _ref1;
        if (v && !_this.fsErrorCache[realpath]) {
          _ref = [null, v], err = _ref[0], res = _ref[1];
        } else {
    ...
    Cache.prototype._loadCacheAndMonitor = function(filename, options, cb) {
      ...
      if (!(this.fsErrorCache[filename] && previous_fs_err && this.fileCache[filename])) {
        if (!fileData) {
          ...
          return cb(this.fileCache[filename] = "Error loading " + filename);
        } else {
          return this.transform(fileData, function(err, transformedData) {
            ...
            if (err) {
              ...
              ret = _this.fileCache[filename] = transformedData;
    ...
    Cache.prototype._reloadFileInBkg = function(filename, options) {
      ...
      return fs.readFile(filename, 'utf8', function(err, fileData) {
        if (err) {
          _this._log("Error when re-reading " + filename + " in background");
          _this.fsErrorCache[filename] = Date.now();
          fileData = null;
        } else {
